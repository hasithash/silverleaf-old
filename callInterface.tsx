...
export default function CallInterface({ startingMessage }: { startingMessage: string }) {
  ...
  useEffect(() => {
    if (status.value === 'connected') {
      visualizerRef.current = new AudioVisualizer('visualizerBox')
  ...
  useEffect(() => {
    if (visualizerRef.current && fft) {
      console.log('FFT data received:', fft);
      if (!fft || fft.length === 0) {
        console.warn('No FFT data available');
        return;
      }
      visualizerRef.current.updateWithFFT(fft);
    }
  }, [fft]);
